-- 광고 표기 판정 오탐 수정(2026-08-28) 재판정 초기화 —
-- feat/monitoring-ad-negation-scope 브랜치, AdDisclosurePatterns 스팬 겹침 축소 + 전각 해시 매칭 +
-- 오타·변형 해시태그·수령 동사 과거형 일반화 대응(운영 전수 스윕으로 확정된 오탐 전부). 이 SQL은
-- 파일만 준비하는 것 — 배포 전에는 절대 실행하지 않는다.
--
...
--   ③ 오타·변형 해시태그(#제픔제공·#재품제공·#제품증정·#상품증정·#제품단순제공) 신규 등재 —
--      08-28 운영 위험 판정 실측 8건, 전부 캡션 상단의 실제 표기 시도. 부정어 공존 여부와
--      무관하게 구 로직이 아예 못 잡던 패턴이라 있으면 대상.
--   ④ 수령 동사 과거형 일반화 — "제품" 접두를 요구하던 "제공받았/받은"류를 "제공받(았|은)"·
--      "증정받(았|은)"으로 확장(08-28 운영 실측 NOT_DISCLOSED "수딩젤도 제공받았는데"류 다수).
--      이 확장은 부정문("제공받은 것 없는 단순 공유입니다")에도 신규 매칭을 만들지만
--      NEGATION 스팬 겹침으로 방어돼 그 부정문 자체는 재판정해도 결과가 같다 — 그래도 정규식
--      자체엔 걸리므로(과대선정 감수) 아래 WHERE에 함께 포함한다(정밀 배제는 안 함).
--
...
-- WHERE는 이 41건 기준에 ③·④ 신규 클래스를 더한 버전이다. 참고 규모(08-28 운영 실측, 클래스간
-- 중복 있어 단순 합산 아님 — 실행 전 0번 SELECT로 최종 건수 확인):
--   ①·② 41건 + ③ 오타 해시태그 8건 + ④ 제공받(았|은)류 ~105건 + 증정받(았|은)류 ~5건.
--
...
--   0) 대상 건수 확인(실행 전 반드시 먼저 SELECT로 확인 — 08-28 운영 실측 41건 + ③·④ 추가분):
...
--              or caption ~ '[#＃](제픔제공|재품제공|제품증정|상품증정|제품단순제공)'
--              or caption ~ '(제공|증정)\s*받(았|은)'
--           );
--   1) 위 건수가 합리적 범위임을 확인한 뒤 본 스크립트 실행:
 ...
 where ad_verdict in ('NOT_DISCLOSED', 'INSUFFICIENT', 'UNCERTAIN')
   and (
     ...
     or caption ~ '[\(\[]\s*(광고|협찬)\s*[\)\]]'
        -- ③ 오타·변형 해시태그(신규 등재)
     or caption ~ '[#＃](제픔제공|재품제공|제품증정|상품증정|제품단순제공)'
        -- ④ 수령 동사 과거형 일반화(신규 등재 — 부정문 2건도 재판정 결과는 동일하지만 정규식
        -- 자체엔 걸리므로 그대로 포함, 정밀 배제는 하지 않는다)
     or caption ~ '(제공|증정)\s*받(았|은)'
   );
