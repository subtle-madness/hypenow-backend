 ...
 * 동형 계약)을 그대로 재사용 — 사이드카 복원만 배치별 파일({@link BatchSidecarStore})로 갈라진다.
...
public class ContentBatchCollectJob {
	...
	public ContentBatchCollectJob(DataSource analysisDataSource, GeminiBatchApi batchApi,
			BeautyTaxonomyLoader taxonomyLoader, AnalyticsSettings settings, Path workDir) {
	...
	public JobResult run() {
		...
		List<Map<String, Object>> pending = analysis.queryForList(
				"SELECT id, batch_name FROM content_batch_jobs WHERE status = 'pending' ORDER BY submitted_at");
		int collected = 0;
		int failed = 0;
		for (Map<String, Object> row : pending) {
			...
			try {
				collected += collectOne(id, batchName);
			} catch (Exception e) {
				failed++;
	...
	private int collectOne(long id, String batchName) {
		...
		if (!state.endsWith("_SUCCEEDED")) {
			...
			analysis.update("""
					UPDATE content_batch_jobs SET status = 'failed', collected_at = now(), note = ?
					WHERE id = ?""", "배치 실패 상태: " + state, id);
		...
		try {
			...
			analysis.update("""
					UPDATE content_batch_jobs SET status = 'failed', collected_at = now(), note = ?
					WHERE id = ?""", "결과 파일 이름을 찾지 못함", id);
			log.warn("배치 결과 파일 이름 판독 불가 — batch_name={}, 응답: {}", batchName, batch);
			return 0;
		}
		Map<String, Map<String, String>> sidecar = BatchSidecarStore.read(workDir, batchName);
		...
		analysis.update("UPDATE content_batch_jobs SET status = 'collected', collected_at = now() WHERE id = ?", id);
		BatchSidecarStore.delete(workDir, batchName);
