...
-- 모수 = **뷰티 인플루언서 ∩ 자격 팔로워 구간**(500 이상 50,000 미만 — crawler qualify.min/max-followers와
-- 정합)과 그 계정들의 ENUMERATION 콘텐츠. 07-29: 하한 3,000 → 500으로 확장(나노 포함) —
-- 랜딩 "보유 풀"이 실제 보유 자격 계정 전체를 세도록 (ARCHITECTURE §7 07-29 결정).
...
WITH micro_account AS (
  ...
  WHERE i.status = 'QUALIFIED' AND i.beauty AND NOT i.beauty_company
    AND p.followers >= 500 AND p.followers < 50000
),
micro_content AS (
...
account_agg AS (
  ...
  SELECT count(*) AS influencers_count,
         count(*) FILTER (WHERE followers < 3000) AS followers500to3k,
         count(*) FILTER (WHERE followers >= 3000 AND followers < 10000) AS followers3k10k,
