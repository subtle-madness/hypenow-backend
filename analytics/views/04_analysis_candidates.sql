...
-- 증분 창(경과 < 'analytics.analyze-window-days'(기본 14)일 — 07-19 확정: 백필 MVP 제외,
-- 데일리 파이프라인은 최근 성숙분만. 분석이 며칠 밀려도 창 안에서 재대상, 창을 넘긴 건 분석 안 함).
...
WHERE v.caption IS NOT NULL AND btrim(v.caption) <> ''
  ...
  AND v.posted_at + make_interval(days => COALESCE(
        (SELECT value::int FROM app_setting WHERE key = 'analytics.analyze-window-days'), 14)) > now();
