...
-- ※ 2026-09-03 리뷰: 우선순위는 완결(기분석) > 차단 사유(gated·미러 갭) > 사실만 > 순수 대기 순.
--   차단 사유를 사실만보다 먼저 평가해야 한다 - 안 그러면 댓글 게이트 보류·미러 갭인 사실행이
--   24/34("파트 B 대기")로만 보여 진짜 차단 사유(22/23/32/33)가 가려진다.
...
SELECT
  ...
  CASE
    ...
    -- 2026-09-03 리뷰: 차단 사유(gated·미러 갭)가 '사실만'보다 먼저 평가돼야 한다 - 순서가 반대면
    -- 댓글 게이트에 막혔거나 미러 갭인 사실행이 24/34("파트 B 대기")로만 보여 진짜 차단 사유가
    -- 가려진다. 완결(기분석, facts_only가 아닌 analyzed)은 그 자체로 최우선 - 지금 차단 신호가
    -- 서 있어도 이미 끝난 콘텐츠라 무관하다.
    WHEN c.timely IS TRUE THEN
      CASE WHEN s.analyzed AND NOT s.facts_only THEN '20'
           WHEN s.gated THEN '22'
           WHEN NOT s.mirrored THEN '23'
           WHEN s.facts_only THEN '24'
           ELSE '21' END
    WHEN c.timely IS FALSE THEN
      CASE WHEN s.analyzed AND NOT s.facts_only THEN '30'
           WHEN s.gated THEN '32'
           WHEN NOT s.mirrored THEN '33'
           WHEN s.facts_only THEN '34'
           ELSE '31' END
...
FROM (VALUES
  ...
  ('22', '랭킹 트랙 · 댓글 게이트 보류', '후보지만 댓글 분류 미완으로 잡이 스킵(파트 A 사실 보유 여부 무관) — 분류 완료 후 자연 재대상'),
  ('23', '랭킹 트랙 · 미러 갭',         '후보지만 미러에 아직 없어 잡이 스킵(파트 A 사실 보유 여부 무관) — 다음 미러 후 자연 해소'),
  ('24', '랭킹 트랙 · 사실만',          '파트 A 완료 · 파트 B 대기, 차단 사유(게이트·미러 갭) 없음 - 광고/카테고리는 뜨지만 랭킹엔 아직 안 나온다'),
  ('30', '상세 트랙 · 기분석',          '제때창 놓침·최근 윈도우 안 + 분석 완료 → 인플루언서 상세만(랭킹 제외)'),
  ('31', '상세 트랙 · 분석 대기',       '분석 대상(백필 잡) — 분석돼도 랭킹엔 안 나온다'),
  ('32', '상세 트랙 · 댓글 게이트 보류', '후보지만 댓글 분류 미완으로 잡이 스킵(파트 A 사실 보유 여부 무관)'),
  ('33', '상세 트랙 · 미러 갭',         '후보지만 미러에 아직 없어 잡이 스킵(파트 A 사실 보유 여부 무관)'),
  ('34', '상세 트랙 · 사실만',          '파트 A 완료 · 파트 B 대기, 차단 사유(게이트·미러 갭) 없음 - 인플루언서 상세엔 이미 반영'),
