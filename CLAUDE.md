...
- **세션 위생**(여러 세션 동시 작업 전제): 작업이 끝나면 머지 준비가 안 됐더라도 **PR을 즉시 연다**(draft
  가능) — 열린 PR이 "이 영역은 점유 중"이라는 세션 간 유일한 신호다. 머지된 worktree·브랜치 정리는
  **`scripts/repo-gc.sh`가 정본**(09-03~): 드라이런이 기본, `--auto`가 "머지됨 + 클린 + 살아있는
  프로세스 없음"만 지운다(스쿼시 머지는 gh PR 상태로 판정). 지침으로만 맡겼더니 07-30 25개 →
  09-03 35개로 늘어 기계화했다 — 정리 요청이 오면 손으로 뒤지지 말고 이 스크립트를 돌릴 것.
  살아있는 세션이 잡은 worktree(LIVE)는 세션을 닫아야 풀린다. 작업이 끝나 PR을 열 때는 그 작업의 plan 문서도 같은 PR에서
