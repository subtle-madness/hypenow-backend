...
public class V1InfluencerDiscoveryRepository {
	 ...
	 *
	 * <p><b>분석 여부와 무관하게 노출</b>(2026-09-04 제품 결정, #749의 "분석 완료(content_analyses
	 * 보유) 게시물만" 필터 원복). 인플루언서 상세의 recentContents(6.4)와 동일 규칙으로 맞춘다.
	 * #749 당시 우려(6.3 {@code GET /v1/contents/{id}/ai-report}가 {@code contents ⋈
	 * content_analyses}(INNER)라 미분석 콘텐츠는 404)는 콘텐츠 분석 성숙 게이트가 2단계(파트A 캡션
	 * D+1 / 파트B 해석 D+4)로 분리되며 6.3이 D+1부터 200을 반환하게 되어, 죽은 썸네일 우려는 당일
	 * 게시물(D+0)에만 남는다 — 그 폭은 수용한다.
	 */
