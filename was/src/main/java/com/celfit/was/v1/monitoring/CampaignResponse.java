 ...
 * 시각을 표시한다(스펙 1.5). nullable 필드(description·startDate·endDate·brand·budget·seedingCount)는
 * 계약 무결성 규칙 #1(1.8)에 따라 키를 생략하지 않고 명시적 null로 직렬화한다 — ApiResponse와 달리
 * 이 record엔 NON_NULL이 없다(기본값). seedingCount는 등록된 추적 행 수와 무관한 유저 선언 입력값
 * (이행률 모수)이라 다른 nullable 필드와 마찬가지로 값을 그대로 보여준다 — 서버가 파생·검증하지 않는다.
 */
public record CampaignResponse(String id, String name, String description, String startDate, String endDate,
		String brand, Long budget, Integer seedingCount, String createdAt) {
