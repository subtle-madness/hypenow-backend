 ...
 *   <li>target_id == null(등록 처리 중) → registered_on + tracking_days가 오늘(KST) 이전이면(만료)
 *       mode=url ? ended : not_uploaded(계약 6.25 "collecting으로 기간이 만료되면 ended로 종결"·
 *       "감지 없이 만료되면 not_uploaded"), 아니면 mode=url ? collecting : detecting</li>
...
public final class ItemStatus {
	...
	/**
	 * @param today KST 기준 오늘 날짜 — target 미확정(pending) 행의 기간 만료 판정에 쓴다. 호출부가
	 *              {@code LocalDate.now(KstTimestamps.KST)}를 넘긴다(순수 함수 유지 목적).
	 */
	public static String derive(MonitoringItemRow item, TargetRow target, LocalDate today) {
		...
		if (item.targetId() == null || target == null) {
			return derivePending(item, today);
		}
	...
	/**
	 * target 미확정(pending) 행 — 등록일 + 기간이 오늘(KST)까지 지났는데도 target이 안 붙었으면 그
	 * 자리에서 종결시킨다. url 모드는 게시물이 이미 특정된 등록이라 ended(collecting 만료 규칙),
	 * account 모드는 아직 아무 게시물도 못 찾은 채라 not_uploaded(detecting 만료 규칙)다.
	 */
	private static String derivePending(MonitoringItemRow item, LocalDate today) {
		LocalDate endDate = item.registeredOn().plusDays(item.trackingDays());
		boolean expired = !endDate.isAfter(today);
		if (expired) {
			return MODE_URL.equals(item.mode()) ? ENDED : NOT_UPLOADED;
		}
