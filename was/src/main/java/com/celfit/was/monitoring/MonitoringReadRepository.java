...
public class MonitoringReadRepository {
	...
	public List<TrackedSnapshotRow> findSnapshots(Collection<String> shortCodes, LocalDate maxCapturedOn) {
		...
		return jdbc.sql("""
				SELECT short_code, captured_on, content_type, likes, likes_hidden, comments, views, saves, shares, reposts
				FROM post_snapshot
	...
	public List<PostSnapshotRow> postTimeseries(long targetId) {
		return jdbc.sql("""
				SELECT captured_on, content_type, likes, likes_hidden, comments, views, saves, shares, reposts
				FROM post_snapshot
