 ...
 * findEngagements/findGroupPurchaseCounts)에 동일하게 전달하는지, total이 countCards를 그대로 싣는지. 컨트롤러
...
class V1InfluencerDiscoveryPageServiceTest {
	...
	void 보강_4쿼리가_모두_같은_handles로_호출되고_total은_첫_행_totalCount다() {
		...
		given(repository.findGroupPurchaseCounts(anyList())).willReturn(List.of());
		...
		ArgumentCaptor<List<String>> groupPurchaseCounts = ArgumentCaptor.captor();
		...
		verify(repository).findGroupPurchaseCounts(groupPurchaseCounts.capture());
		...
		assertThat(groupPurchaseCounts.getValue()).isEqualTo(expected);
	}
