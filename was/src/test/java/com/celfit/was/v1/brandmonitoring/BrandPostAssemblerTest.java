...
class BrandPostAssemblerTest {
	...
	void withComments_false면_댓글_배치_조회를_돌리지_않는다() {
		...
		var posts = assembler.assembleBrandPosts(7L, account, false, BrandPostAssembler.BrandPostScope.ALL, BrandAccountType.OWN);
	...
	void direct_등록_행이_없으면_원장_조회를_생략한다() {
		...
		var assembler = newAssembler(repository, campaignRepository, directRepository, trackingAssembler,
				itemRepository, false);
		assembler.assembleBrandPosts(7L, account, false, BrandPostAssembler.BrandPostScope.ALL, BrandAccountType.OWN);
	...
	void scope가_enrichedOnly_인자를_가른다() {
		...
		assembler.assembleBrandPosts(7L, account, false, BrandPostAssembler.BrandPostScope.ENRICHED_ONLY, BrandAccountType.OWN);
