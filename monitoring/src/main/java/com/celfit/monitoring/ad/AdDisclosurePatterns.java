...
public final class AdDisclosurePatterns {
	...
	// "(제공|증정|선물)받(은|는|을)? (것|거|건)? 없"(08-28 추가) — "제공받은 것 없는 단순
	// 공유입니다"(운영 2건 실측)가 위 "제공\s*받(았|은)" 신규 패턴에 false DISCLOSED 되는 걸
	// 막는다. "제공받은"과 "없"이 이어지는 스팬이 Tier1 후보 스팬과 겹쳐 그 후보만 제외된다.
	private static final Pattern NEGATION = Pattern.compile(
			"내돈내산|(광고|협찬)\\s*(이|가|은|는)?\\s*(아니|아님)|(제공|증정|선물)\\s*받(은|는|을)?\\s*(것|거|건)?\\s*없");
