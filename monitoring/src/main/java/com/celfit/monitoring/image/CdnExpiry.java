...
final class CdnExpiry {
	...
	/**
	 * 만료 임박 순 정렬 — 배치 상한이 걸릴 때 남은 예산을 "먼저 죽을 URL"에 쓴다(임박분을 이월하면
	 * 다음 스윕엔 이미 만료돼 영구 유실). 만료 미상(oe 없음)은 맨 뒤 — 지금 안 잡아도 다음 스윕에
	 * 그대로 남아 있을 가능성이 높으므로 임박한 것에 양보한다. 만료 시각은 여기서 한 번만 계산해
	 * {@link Ranked}에 실어 반환한다 — 호출부는 {@code r.expired(now)}로 판정하고 URL을 재파싱하지
	 * 않는다.
	 */
