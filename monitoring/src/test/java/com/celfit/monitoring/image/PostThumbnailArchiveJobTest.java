...
class PostThumbnailArchiveJobTest {
	...
	void 신규_아카이브는_object_path_source_name_archived_at을_기록한다() {
		seedPostMeta("SC1", "https://cdn.example/v/t51/463_111_n.jpg?" + CdnUrls.farFutureOe(), null, null);
	...
	void source_name이_바뀌지_않으면_재다운로드를_스킵한다() {
		seedPostMeta("SC1", "https://cdn.example/v/463_111_n.jpg?" + CdnUrls.farFutureOe(),
	...
	void 쿼리스트링만_다르고_파일명이_같으면_스킵한다() {
		seedPostMeta("SC1", "https://cdn-a.example/v/999_222_n.jpg?" + CdnUrls.farFutureOe() + "&sig=1",
	...
	void 파일명이_바뀌면_같은_키로_재업로드하고_source_name을_갱신한다() {
		seedPostMeta("SC1", "https://cdn.example/v/999_222_n.jpg?" + CdnUrls.farFutureOe(),
				"monitor-post/SC1.jpg", "999_222_n.jpg");
		db.update("UPDATE post_meta SET thumbnail_url = ? WHERE short_code = 'SC1'",
