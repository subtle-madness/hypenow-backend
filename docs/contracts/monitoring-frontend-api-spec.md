...
| recentComments | object[] | 아래 PostComment. 최신순, 수집 상한까지만(전체 개수는 최신 Snapshot의 comments). 첫 수집 전이면 빈 배열 |
...
| reply | object 또는 null | `{ text }`. **게시물 작성자(인플루언서) 본인의 답글만** 담는다. 제3자 답글은 수집·응답 대상이 아니다. 답글 작성 시각은 화면 소비처가 없어 계약에서 제외한다 |
...
마스킹은 응답 생성 단계의 서버 책임이다. 개인정보처리방침 제3장 ②에 "댓글 작성자의 계정명은 서비스 화면에서 일부 마스킹하여 표시합니다"로 이미 공개돼 있어, 원본 핸들이 프론트로 전달되면 방침 위반이다 `[확인 필요]`(수집 상한 개수와 마스킹 규칙 확정 필요, 4절 28번).
...
| 28 | 댓글 원문 수집 | **결정(2026-07-29): 작성 시각·인플루언서 답글 모두 수집 가능** | 남은 확인: 게시물당 수집 상한(프론트 mock은 8개)과 수집 주기. 기존 `content_comments`에 시각·답글 컬럼 추가 필요 |
