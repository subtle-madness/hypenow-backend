...
## 0-1. 프론트 구현 시 주의사항 (요약)

상세는 각 절에 있고, 여기는 놓치기 쉬운 것만 모았다.

1. **본문은 글자 단위로 오지 않는다.** SSE에서도 답변 본문은 턴이 완성된 뒤 `delta` 한 덩어리로 온다(§4.3 홀드백). 그동안은 `status` 이벤트의 `label`("생각하는 중", "게시물 찾는 중", "답변 정리하는 중")을 로딩 문구로 보여줘야 체감이 맞다. 툴 호출이 여러 번이면 `thinking`과 `tool`이 번갈아 여러 번 온다.
2. **연결을 끊어도 사용량은 차감된다.** 사용자가 답변 도중 화면을 벗어나 SSE가 끊기면 그 질문은 실패가 아니라 중단으로 기록되고 일일 횟수에서 차감된다(§4.4). "취소" 버튼을 둔다면 이 점을 안내하거나, 완결 JSON 경로도 같은 규칙임을 감안한다.
3. **응답 시간 계약은 90초.** 그 안에 예산이 다하면 부분 답변이 오고 `limitReached`가 `"time"` 또는 `"budget"`으로 채워진다. 정상이면 `null`. 부분 답변임을 알리는 표시가 필요하다(§3.3).
4. **`accountIds`는 정확히 1개.** 다중 계정 비교는 계약 밖이다. 같은 `conversationId`로 다른 브랜드를 보내면 409 `CONVERSATION_SCOPE_MISMATCH`. 브랜드 선택을 바꾸면 새 대화를 시작해야 한다.
5. **검증 순서.** 형식·소유권 오류(400/404/409)가 분당·일일 한도(429)보다 먼저 걸린다(§3.5). 429는 유효한 요청에서만 나오며, 남은 횟수는 `GET /ai/usage`로 미리 보여줄 수 있다(기본 일일 30·분당 10, §8).
6. **`messageId`는 null일 수 있다.** 로그 적재 실패 시 null이 온다(§3.3). 이때는 피드백 버튼을 비활성화해야 한다(피드백 API가 messageId를 경로로 받는다, §7).
7. **`followUps`는 정확히 2개 또는 빈 배열.** 1개가 오는 경우는 없다. 빈 배열이면 후속 질문 영역을 숨긴다(§10.2). 후속 질문을 누르면 그 `text`를 같은 `conversationId`로 보낸다.
8. **`references`는 실제 인용된 게시물만, 최대 10개.** 본문에 shortCode가 문자열로 병기되지만(§10.1) 본문을 링크화할지, references 카드만 보여줄지는 FE 선택이다. 자동 링크화는 하지 않는 편이 안전하다(캡션 인용에 URL이 섞일 수 있다).
9. **마크다운은 허용 범위가 좁다**(§10). 굵게·인라인 코드·표(5열 이하)·인용·번호/불릿·구분선만 생성한다. 모델 실수로 헤더나 링크가 섞여 와도 렌더러가 깨지지 않게 방어적으로 렌더한다(헤더는 평문 취급 등).
10. **대화 상세의 `feedback`은 항상 존재하고 값이 null이거나 객체다.** `messageId`는 assistant 메시지에만 있다(§6.2). 삭제된 대화의 메시지에 피드백을 보내면 404다.
11. **프리셋은 id만 계약이다.** 버튼 문구는 FE가 `text`로 보내는 값이고 §5.1의 문구는 권장안이다. 구 5종 id는 당분간 동작하지만 제거 예정이니 전환 시점을 백엔드에 알려준다.
12. **에러 응답의 `error.message`는 사용자에게 그대로 보여줄 수 있는 한국어 문장**이다(예: "메시지가 너무 길어요. 더 짧게 나눠서 물어봐 주세요."). 별도 매핑 없이 노출해도 된다.
