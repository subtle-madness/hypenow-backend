...
- **인덱스 캐시 힙 예산(08-28 실측, DECISIONS 08-28 행)** — ref당 890바이트. 운영 최대 브랜드는
  **계정 120(창 안 10,489행, 119의 2배)** 이라 엔트리 하나가 ~9MB, LRU 8개로 최악 ~73MB.
  운영 was 힙은 2GB에 Old Gen p99 798MB·최대 887MB(Prometheus 7일)라 여유는 있지만 한가하지 않다.
  **브랜드 모수가 지금보다 크게 늘거나 동시 사용 유저가 8명을 넘어서면 상한·구조를 재검토할 것**
  (엔트리는 유저 × 브랜드 × 종류로 갈린다). 힙 재측정 레시피는 메모리 노트 참조.
