...
2. **`signup_events` 재식별 경로**(설계 §4-4) — `users`를 가명화해도 `app.signup_events`가
   ...
   **(09-03 갱신, 트랙 A)** 이후 `signup_events`에 90일 보존 배치(`SignupEventRetentionScheduler`)가
   추가돼 "삭제 경로 없음" 전제는 깨졌다 — 재식별 창은 90일로 한정되고, 그 이상은 재식별 편의보다
   개인정보 최소 보존을 우선한 의도된 결정이다. 자세한 내용은 트랙 QQ 문서·DECISIONS.md 참조.
