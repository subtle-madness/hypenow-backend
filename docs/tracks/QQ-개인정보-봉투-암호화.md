...
4. **DEK 에스크로 — ✅ 완료(2026-09-04)**: 운영 `encryption_keys` key_id=1의 래핑본을 관리자 계정
   `oci kms crypto decrypt`로 언래핑해(88자 base64 = 64바이트) 운영자 비밀번호 관리자(맥 암호 앱)에
   보관. 스테이징 DEK는 대상 아님. DEK 로테이션 시 갱신 필요. 아래는 결정 근거·절차 원문.
   **(원문) PR 3(평문 DROP) 전에 반드시 수행** (09-03 결정: 방안 A). 키 계열 비대칭
