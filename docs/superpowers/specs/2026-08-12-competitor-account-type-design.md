...
## 부록 (08-12 추가) — 세 번째 신규 코드 `COMPETITOR_ACCOUNT_NOT_ALLOWED`

> 위 본문은 설계 시점 기록이라 손대지 않는다. 구현 중 코드가 하나 더 늘었고 그 결과 §7 「한계
> (의도적)」의 적용 범위가 본문보다 좁아졌으므로, 아래를 본문에 우선하는 정정으로 읽는다.

### 어디서 나는가

`POST /v1/brand-monitoring/accounts/{accountId}/direct-posts` (브랜드 직접 등록 접수) —
`V1BrandDirectPostService.requireOwnership`. 연결 자체는 있으나 그 연결의 `accountType`이
`competitor`면 **403 `COMPETITOR_ACCOUNT_NOT_ALLOWED`**("경쟁사 계정의 게시물은 추적 등록할 수
없어요.")로 거절한다. 연결이 아예 없을 때의 403 `FORBIDDEN`과 코드를 나눈 것은 FE가 "권한 없음"과
"경쟁사라서 안 됨"을 다른 문구로 안내해야 하기 때문이다.

따라서 이 설계가 신설한 코드는 **셋**이다 — 「결정 요약」표의
`COMPETITOR_ACCOUNT_LIMIT_REACHED`(409)·`COMPETITOR_CONTENT_NOT_ALLOWED`(건별 실패) + 이 403.

### 왜 늘었나

§7의 캠페인 방어만으로는 우회가 남는다. 캠페인 추가(`add()`)의 첫 갈래는 "이미 내 추적 아이템이면
캠페인만 연결"이라 경쟁사 게시물을 **직접 등록으로 먼저 아이템화**해 두면 §7의 판정에 걸리지 않고
그대로 통과한다. 등록 경로를 막지 않으면 §7이 막으려던 것을 그대로 되돌려 준다.

§7이 "판정할 근거가 없다"고 본 것과 이 경로가 다른 이유: 직접 등록은 **어느 브랜드에 등록하는지를
요청이 스스로 지목한다**(`brand_direct_posts.brand_id` × 그 링크의 `accountType`). 게시물 작성자
계정을 추론할 필요가 없어 판정 근거가 요청 안에 이미 있다. 성과 대시보드도 같은 매핑을 근거로 그
콘텐츠를 경쟁사로 분류해 기본 범위에서 빼므로, 등록만 열어 두면 두 표면이 같은 콘텐츠를 두고 서로
다른 말을 한다.

### §7 「한계 (의도적)」의 범위 축소

§7의 한계 문단은 이제 **개인 URL 추적(레거시 `/v1/monitoring` 계열 직접 등록)에만** 적용된다.
거기서는 여전히 게시물 작성자를 경쟁사 구독과 대조하지 않으며, 그 자유도를 깎지 않는다.
**브랜드 직접 등록(`/v1/brand-monitoring/accounts/{id}/direct-posts`)은 한계가 아니라 차단
대상**이다 — 위 근거로 판정이 가능하고, 실제로 403으로 막는다. §7 문단이 "직접 등록 경로는
막지 않는다"로 읽히면 그것은 구현과 반대이므로, 이 부록이 이긴다.

### 지금 넣어도 안전한 이유

`accountType`은 08-12 신설이고 기존 연결은 마이그레이션 기본값으로 전량 `own`이라, 배포 시점에
경쟁사 구독을 가진 유저가 존재하지 않는다. 즉 이 403으로 깨지는 기존 흐름이 없다.
