...
| 6 | ~~컨테이너 메모리 제한 도입~~ → **보류** (07-19 사용자 방침 "지금 최적화는 하지 않는다") | §2 예산표는 참고치로 보존 — 규모 확장·자동화 시점에 별도 승인으로 도입 |
...
※ 아래 메모리 표는 **참고 예산치** — 실제 `mem_limit` 적용은 보류(§1-6). 지금 적용되는 건 crawler의 힙 지정(-Xmx1g)뿐.
