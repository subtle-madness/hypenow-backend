...
| 8 | **게시물 이미지는 OCI Object Storage + Vercel rewrite 캐시** | 인스타 CDN URL ~4일 만료 대응 + FE 과거 기간 화면의 전제. 수집 시 리사이즈(WebP ~320px) → 버킷 업로드, FE는 `vercel.json` rewrite `/img/*` + 콘텐츠 해시 파일명·`immutable` 캐시로 엣지 캐싱(변환 쿼터 미사용). Cloudflare CDN은 네임서버 이전이 필수라 배제. 무료 한도: 무료 전용 계정 합산 20GB·월 5만 요청 — 1,500계정 규모(신규 게시물 ~700개/일 실측)는 업로드 월 ~2.1만 건·연 4~10GB로 성립. 2만 규모는 월 ~28만 건·연 50~135GB로 PAYG 전환이 전제(초과 과금 월 수 달러 — 단 PAYG 시 Standard 무료는 10GB로 축소, IA·Archive 각 10GB 별도) |
...
7. **Object Storage 버킷 준비** — 이미지용 버킷 생성(OCI CLI), FE 서빙용 읽기 접근 정책,
   crawler 업로드용 자격 증명 구성. 이미지 파이프라인 코드(다운로드·리사이즈·업로드)와
   FE rewrite는 별도 태스크(§7) — 이번엔 "이미지가 들어갈 자리"까지만 인프라로 완성.
...
- 신규 게시물 생성률: 1,500계정/일 방문에서 ~700개/일 (계정당 ~0.47개/일) — 이미지 용량·요청 추계의 근거
