...
**2단계 PR로 나눠야 한다** — 최초 설계의 "한 PR로 가능"은 틀렸고 2026-08-27 실측으로
정정한다. expand만이라 롤링 자체는 안전하지만, **컬럼을 만드는 crawler와 그 컬럼을 읽는
analytics 뷰가 서로 다른 배포 경로를 탄다**: cd-test(staging)는 crawler를 배포하지 않으므로
컬럼은 staging→main 승격 배포로만 서버 raw DB에 생긴다. 한 PR로 묶으면 develop→staging
단계에서 analytics가 컬럼 없는 DB에 `v_base_influencer`를 적용하다 깨진다(2026-08-27
run 33108790862 실측 — PR #615로 뷰 노출·시드·ASSERT만 임시 리버트해 언블록). F&B 축도
같은 이유로 노출이 별도 후속 커밋이었다(08-25 전례).

- **1단계**: 마이그레이션(컬럼+토글 시드) + crawler·was 코드 → develop → staging → main.
  main 배포로 서버 raw DB에 컬럼이 생긴다(V20260827090717은 2026-08-30 적용 확인).
- **2단계**: analytics 뷰 노출(`v_base_influencer`의 home_living 3컬럼) + 더미 시드 · SQL
  하니스 ASSERT → develop → staging. **컬럼 존재를 서버에서 확인한 뒤에만 연다.**

배포 직후부터 새벽 판정 크론이 신규 3축 판정 + 홈/리빙 백필을 자동 시작(여러 날 소화).
수집 편입은 모수·비용 확인 후 토글 on.
