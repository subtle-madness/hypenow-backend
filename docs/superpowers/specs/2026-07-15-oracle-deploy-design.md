...
- 서버 cron: **매일 `pg_dump` → 압축 → 서버 7일 롤링 + rclone으로 Google Drive 업로드(30일 롤링)** —
  맥이 꺼져 있어도 오프사이트 사본이 무인 생성된다. 오프사이트를 오라클 Object Storage가 아닌
  구글에 두는 이유: 계정 회수 시 본체와 같이 사라지면 안 되므로. `rclone.conf`는 맥에서 1회
  OAuth 설정 후 서버로 복사.
- 맥으로 최신 덤프를 pull하는 스크립트는 보조(수동·복원 리허설용).
...
- 백업 덤프를 로컬 postgres에 복원해 무결성 확인 (복원 리허설 1회) + Google Drive에 덤프 도착 확인.

## 8. 후속 — 2단계: 크롤·분석 파이프라인 클라우드 이전 (별도 태스크)

1단계(이 스펙) 안정화 후, crawler·analytics를 **같은 인스턴스**로 이전해 매일 크론
(크롤 → 분석 배치 → 미러)을 무인화한다. 12GB 여유 내라 추가 비용 $0. 기대 효과:
맥 의존 제거, 썸네일 ~4일 만료 전 분석 보장(ARCHITECTURE §6 제약), 미러 갱신 주기
미결(ARCHITECTURE §8) 해소. raw DB 이사(pg_dump 1회)·크론 설계·API 키 서버 반입
(LLM 배치는 게이트 off 기본 + 크론 명시 플래그 유지 — 비용 가드)이 포함되며 별도 스펙/계획으로 진행.
