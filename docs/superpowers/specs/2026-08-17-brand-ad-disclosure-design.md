...
- **캡션 수준 부정 가드**: `내돈내산`, `광고 아니`, `협찬 아니`, `광고아님`, `협찬아님` 같은
  부정·자비 구매 신호가 캡션 어디든 있으면, 다른 고신뢰 패턴이 매칭돼도 Tier 1은
  확정을 포기하고 Tier 2(LLM)로 넘긴다. 이건 **NOT_DISCLOSED 확정이 아니라 판단
  보류**다 — "내돈내산이지만 #광고"처럼 문맥이 필요한 캡션의 최종 판정은 문맥을 읽을
  수 있는 LLM 몫으로 남긴다.
