...
- **캡션 수준 부정 가드**: `내돈내산` 또는 `(광고|협찬)\s*(아니|아님)`(붙여쓰기·띄어쓰기,
  "아니"/"아님" 두 활용형 모두 커버) 같은 부정·자비 구매 신호가 캡션 어디든 있으면,
  다른 고신뢰 패턴이 매칭돼도 Tier 1은 확정을 포기하고 Tier 2(LLM)로 넘긴다. 이건
  **NOT_DISCLOSED 확정이 아니라 판단 보류**다 — "내돈내산이지만 #광고", "#광고 아님
  사비로 구매"처럼 문맥이 필요한 캡션의 최종 판정은 문맥을 읽을 수 있는 LLM 몫으로
  남긴다. (`광고\s*아니` 같은 2음절 리터럴만으로는 "광고 아님"의 "아님"을 못 잡는
  구멍이 있었다 — `(아니|아님)` 대안으로 교체해 해소.)
