...
- 합류자의 `repo_ms`는 0에 가깝다(자기 스레드에서 리포지토리를 안 탄다) — 합류가 실제로 일어났다는
  직접 증거로 쓴다. **정확히 0은 아니다** — 합류자도 버전키 계산의 리포지토리 콜은 자기 스레드에서
  탄다. 또한 조립이 1초 아래로 떨어지면 `SlowRequestStageLogFilter`(임계 1초)가 아예 로그를 남기지
  않으므로, **느린 요청 로그가 사라지는 것 자체가 성공 신호**다(그때는 표본이 없다는 뜻이지 관측
  실패가 아니다).
- **5xx 비율도 함께 본다** — §2-3이 고른 "리더 실패는 합류자에게도 전파" 때문에 일시적 DB 오류 1건이
  이제 요청 7건을 한꺼번에 5xx로 만든다(FE에서는 패널 하나가 아니라 대시보드 전체가 깨져 보인다).
  의도한 트레이드오프지만(각자 재시도하면 DB가 힘든 그 순간에 N중 재조립이 터진다) 배포 후 실제
  빈도는 관측 대상이다.
