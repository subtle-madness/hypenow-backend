...
### 3-2. 크롤러 몫: `request_count IS NOT NULL`인 실행만

Apify 실행과 무료 소스는 이 조건으로 자연히 빠진다(§1). 별도 액터 라벨 화이트리스트를 두지
않는 이유가 이것이다 — 새 무료 경로가 생겨도 규칙을 고칠 필요가 없다.

`status`는 거르지 않는다. `request_count`는 `finishOk` 경로에서만 기록되고, 요청이 나간 뒤
...
"비용 상한 관점"). 따라서 **유저별 카드를 전부 더한 값 > 이 API의 합계**가 정상이며, 실제로
나간 돈은 이 API 쪽이다. 계약 문서와 화면 툴팁 양쪽에 명시한다.
...
- **SQL 하니스** `analytics/test/30_crawl_cost.test.sql` — KST 자정 경계(23:59:59Z / 00:00:01Z 쌍),
  `request_count IS NULL` 제외(Apify·self 실행), 실패 실행의 `request_count` 유지분 포함,
  잡별 그룹핑.
