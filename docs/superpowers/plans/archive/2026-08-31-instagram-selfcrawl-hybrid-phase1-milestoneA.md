> 상태: ✅ 구현됨 · 마일스톤 A 전 10태스크 완료 · 회귀 통과(instagram-source 69 / monitoring 783, 0 failures) (2026-08-31)
> 범위: Phase 1 마일스톤 A(안전 리팩터 — 신 모듈 + monitoring seam 결선, **행동 변화 0**).
> 구현: branch `claude/optimistic-knuth-3212ef` 커밋 489e4a4e~84358784. 후속 마일스톤 B·C는 `plans/`에 활성 유지.
...
- [x] **Step 1: `settings.gradle`에 모듈 등록**
...
- [x] **Step 3: `package-info.java` 생성(스코프 규약)**
...
- [x] **Step 1: monitoring이 모듈에 의존하도록 build.gradle 수정**
...
- [x] **Step 3: 이동한 5파일의 package 선언 교체**
...
- [x] **Step 4: 모듈에서 DTO 컴파일 확인**

Run: `./gradlew :instagram-source:compileJava`
Expected: `BUILD SUCCESSFUL` (DTO 5종 + package-info).

- [x] **Step 5: monitoring 전역 import 재작성**
...
- [x] **Step 6: 잔류 hiker 파일에 DTO import 추가(컴파일이 안내)**
...
- [x] **Step 7: `PostInfoTest`를 모듈로 이동**
...
- [x] **Step 8: 모듈 테스트 + monitoring 컴파일 gate**
...
- [x] **Step 4: monitoring 전역 import 재작성(예외·HikerHttp·ShortCodes)**
...
- [x] **Step 5: 잔류 hiker 파일에 import 추가(컴파일이 안내)**
...
- [x] **Step 6: `ShortCodesTest`를 모듈로 이동**
...
- [x] **Step 1: 5개 top-level record 파일 생성(본문은 현 중첩 정의 verbatim)**
...
- [x] **Step 3: `HikerClient`에서 중첩 공개 record 정의 삭제 + import 추가**
...
- [x] **Step 1: 인터페이스 생성(현 HikerClient 공개 표면 그대로)**
...
- [x] **Step 5: 픽스처가 다른 monitoring 테스트에서도 쓰이는지 확인**
...
- [x] **Step 3: `FailoverInstagramSource` 구현(전부 위임)**
...
- [x] **Step 2: 컴파일 확인(소비자는 아직 HikerClient 참조 — 다음 Task에서 해소)**
...
- [x] **Step 3: 테스트 코드의 HikerClient 참조 교체**
...
- [x] **Step 1: `HikerClient` 심볼 완전 소멸 확인**
...
- [x] **Step 2: hiker 패키지 잔류 파일 확인(딱 7개여야 함)**
...
- [x] **Step 3: 모듈 경계 규칙 확인(모듈에 Spring·DB 없음)**
...
- [x] **Step 5: monitoring 전체 테스트(회귀 — 행동 변화 0 검증)**
...
- [x] **Step 7: 자기 검토(writing-plans Self-Review)**
...
- [x] `instagram-source` 모듈이 `settings.gradle`에 등록되고 독립 컴파일·테스트된다.
- [x] DTO 5종·결과 record 5종·전송 인터페이스·예외 6종·`ShortCodes`·파싱(`HikerBackend`)이 모듈에 있다.
- [x] `InstagramSource` 인터페이스와 `FailoverInstagramSource`(Hiker 단독 위임)가 있다.
- [x] monitoring이 `implementation project(':instagram-source')`로 의존하고, `HikerConfig`가 `InstagramSource` 빈을 조립한다.
- [x] 소비자 10곳이 `InstagramSource`를 주입받는다. `HikerClient` 심볼은 코드베이스에서 0건.
- [x] `:monitoring:test` 전체가 착수 전과 동일하게 PASS(**행동 변화 0**).
- [x] 모듈에 Spring·DB 의존 없음(경계 규칙 새 예외 성격 유지).
- [x] 이 계획 문서를 실행 완료 시 `docs/superpowers/plans/archive/`로 이동(CLAUDE.md 세션 위생).
