...
- [x] V6 마이그레이션 (시드는 빈 값 — fail-closed로 시작, 운영자가 UPDATE로 개통):
...
- [x] AppSettingRepository (기존 JdbcClient 관용구 — UserRepository 스타일):
...
- [x] 테스트(UserRepositoryTest와 같은 베이스 사용 — 실행 시 해당 파일 패턴 확인 후 동일하게):
  시드 행 존재(`signup.code` → 빈 문자열), 없는 키 → empty Optional. UPDATE 후 재조회 시 새 값.
- [x] `./gradlew :was:test --tests '*AppSettingRepositoryTest'` 그린 → 커밋 `feat(was): app.app_setting V6 + 조회 리포지토리`
...
- [x] SignupRequest 첫 컴포넌트로 `String signupCode` 추가(toNewUser는 무변경).
- [x] V1AuthController: `AppSettingRepository` 주입 + slf4j Logger 추가, signup에서 레이트리밋 직후 호출:
...
- [x] 테스트 먼저: V1AuthControllerTest에 `@MockitoBean AppSettingRepository` 추가,
...
- [x] `./gradlew :was:test --tests '*V1AuthControllerTest'` 그린 → 커밋 `feat(was): 가입 코드 검증 — 403 INVALID_SIGNUP_CODE·fail-closed`
...
- [x] LoginWallIntegrationTest (IntegrationTest 상속 + @AutoConfigureMockMvc) — 통합으로 검증해야
...
- [x] `./gradlew :was:test --tests '*LoginWallIntegrationTest'` 그린(다른 테스트는 Task 5까지 빨간 상태 허용)
...
- [x] AuthController.signup 삭제. auth.SignupRequest·UserRepository.insert는 grep으로 사용처 확인 후 미사용이면 삭제.
- [x] AuthFlowIntegrationTest는 레거시 표면(/api/auth·/api/me) 계약 테스트 — 월 이후 레거시는 전부 잠겨
  죽은 표면이므로 **삭제**하고, 세션·CSRF의 실질 커버리지는 v1 통합 테스트(SessionPersistence·CsrfCookieFlow,
  Task 5에서 /v1/auth 경로로 전환)가 승계한다.
- [x] 커밋 `feat(was): 레거시 /api/auth/signup 폐쇄 — 가입 입구 /v1 일원화`

### Task 5: 기존 테스트 일괄 보강 (가장 잔손 많음)

**Files:** 슬라이스 16개 중 잠긴 경로를 치는 전부 + 통합 테스트 3종.

- [x] **슬라이스(읽기 컨트롤러)**: contentlist·postdetail·postdemo·coverage·influencer(구)·
...
- [x] **V1GateEventControllerTest**: 익명 그대로 통과해야 한다 — 수정 없이 그린인지만 확인(월 회귀 가드).
- [x] **통합 테스트**: SessionPersistence·CsrfCookieFlow·SavedApi가 레거시 /api/auth/login으로 세션을 만들면
...
- [x] `./gradlew :was:test` 전체 그린 → 커밋 `test(was): 로그인 월 반영 — 슬라이스 인증 주입·통합 로그인 /v1 전환`

### Task 6: 문서·전체 검증·PR

- [x] ARCHITECTURE §3 app 스키마 표에 `app_setting` 추가, §7 결정 기록 1행(로그인 월+가입 코드, 스펙 링크).
- [x] 스펙 상태 헤더 → `✅ 구현/실행/반영됨`. 계획 문서 → `plans/archive/`로 이동.
- [x] `./gradlew build` 전체 그린 + bootRun 실기동으로 익명 401/로그인 후 200/코드 가입 실검증.
- [x] push → develop 대상 PR(운영 절차 — app_setting UPDATE·프론트 signupCode 배포 순서 — 본문에 명시).
