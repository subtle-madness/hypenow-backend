...
- **모듈은 서로 import 하지 않는다.** 예외는 공유 모듈 두 개뿐:
  ...
  - **`common-llm`**(2026-08-18 신설) — LLM 전송 계층만(Vertex AI HTTP 호출·SA 토큰 발급·
    재시도/백오프·에러 매핑), Spring 의존 금지. monitoring이 소비하며, 2026-08-27 브랜드
    모니터링 AI 어시스턴트 PoC부터 was도 소비한다(function calling 페이로드도 그냥 JSON
    본문이라 `VertexHttpTransport.post(path, jsonBody)`에 확장 없이 얹힌다). analytics는
