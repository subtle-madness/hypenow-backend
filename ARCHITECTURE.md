...
| J | 서빙 이미지 아카이브 | CDN 만료(~4일) 전 프로필·릴스 썸네일·게시글 썸네일을 OCI `hypenow-images` 버킷에 적재하는 analytics 잡 + `image_assets`(미러 제외 누적) + was COALESCE `/img/` 상대경로 서빙(Vercel rewrite 엣지 캐시 — 프론트 배포 완료) — [specs/2026-07-21-image-archive-design.md](docs/superpowers/specs/2026-07-21-image-archive-design.md) [plans/2026-07-21-image-archive.md → archive] | B1(미러), 어드민 I | ✅ (운영 개통 대기 — 버킷 공개·PAR 등록) |
