...
- raw 변경 감지: `analytics/test/run.sh` 하니스 — 시드가 raw에 직접 INSERT하므로 사실상 계약 테스트.
  CI `sql-harness` 잡으로 연결됨(07-18 — 프레시 Postgres + 마이그레이션 전체 적용 후 실행).
...
| 항목 | 상태 |
|---|---|
| ~~계약 테스트 CI 연결~~ | **해소(07-18)** — 뷰·하니스 신스키마 전환으로 프레시 DB 재현 가능해져 ci.yml `sql-harness` 잡 연결(프레시 Postgres + crawler 마이그레이션 번호순 적용 + run.sh, develop 대상 push·PR마다 실행) |
